...
The images for the character cards are sourced from another API (Akabab) on GitHub. For the other pages, the images are fetched from JSON files. I couldn’t find any API providing these images, so I created three JSON files and manually added the image URLs.